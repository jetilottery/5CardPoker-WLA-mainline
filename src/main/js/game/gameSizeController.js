...
define([
    ...
    function onGameParametersUpdated(){
        ...
        // Let other components know if we need to use portrait
        msgBus.publish('updatePortraitNeeded', needPortrait);
    }

    function updateBG(){
        let spineSymbol, style;
        if(!gr.lib._BGpannel.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.backgroundAmbience.spineData);
			style = {x:720, y:0,scaleX:1, scaleY:1};
			gameUtils.setSpineStype(spineSymbol, style);
            spineSymbol.alpha = 0.5;
			gr.lib._BGpannel.pixiContainer.addChild(spineSymbol);
			gr.lib._BGpannel.spine = spineSymbol;
			spineSymbol.state.setAnimationByName(0, 'ambience_Landscape', true);
        }
        if(!gr.lib._BGpannel_P.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.backgroundAmbience.spineData);
            style = {x:405, y:180,scaleX:1, scaleY:1};
            gameUtils.setSpineStype(spineSymbol, style);
            spineSymbol.alpha = 0.5;
            gr.lib._BGpannel_P.pixiContainer.addChild(spineSymbol);
            gr.lib._BGpannel_P.spine = spineSymbol;
            spineSymbol.state.setAnimationByName(0, 'ambience_Portrait', true);
        }
        if(!gr.lib._transitionContainer.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.Transition.spineData);
			style = {x:0, y:0,scaleX:1, scaleY:1, alpha:1};
			gameUtils.setSpineStype(spineSymbol, style);
			gr.lib._transitionContainer.pixiContainer.addChild(spineSymbol);
			gr.lib._transitionContainer.spine = spineSymbol;
            // spineSymbol.state.setAnimationByName(0, 'landscape_Transition', true);
            spineSymbol.state.setEmptyAnimation(0);
            spineSymbol.skeleton.setToSetupPose();
            spineSymbol.update(0);
        }
        if(!gr.lib._transitionContainer_P.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.Transition.spineData);
			style = {x:-315, y:202.5,scaleX:1, scaleY:1, alpha:1};
			gameUtils.setSpineStype(spineSymbol, style);
			gr.lib._transitionContainer_P.pixiContainer.addChild(spineSymbol);
            gr.lib._transitionContainer_P.spine = spineSymbol;
            spineSymbol.state.setEmptyAnimation(0);
            spineSymbol.skeleton.setToSetupPose();
            spineSymbol.update(0);
			// spineSymbol.state.setAnimationByName(0, 'portrait_Transition', true);
        }
        if(!gr.lib._winParticles.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.totalWin.spineData);
			style = {
                landscape: {x:720, y:390,scaleX:1.2, scaleY:1.2, alpha:1},
                portrait: {x:405, y:560,scaleX:1.2, scaleY:1.2, alpha:1}
            };
			gameUtils.setSpineStype(spineSymbol, style[SKBeInstant.getGameOrientation()]);
			gr.lib._winParticles.pixiContainer.addChild(spineSymbol);
            gr.lib._winParticles.spine = spineSymbol;
            spineSymbol.state.setEmptyAnimation(0);
            spineSymbol.skeleton.setToSetupPose();
            spineSymbol.update(0);
			// spineSymbol.state.setAnimationByName(0, 'portrait_Transition', true);
